(function() {
    ...
    var prevBtn;    // prev btn pressed
    
    var mathOp; // math operation
    
    var prevMathOp; // prev math operation
    
    var mathOpCount;    // math operation counter
    
    var mathOpPress;    // math op pressed?
    
    var isInit; // init
    
    var mainScreen = document.querySelector('#main');   // main screen
    
    var historyScreen = document.querySelector('#history'); // history screen
    ...
    function handler(btn) {
        ...
        if (!isNaN(btn) || btn === '.') {
            ...
            if (btn === '%') {
                ...
                console.log(history);
                history = (btn === '1/x' ? '1/(' + history + ') ' : btn + '(' + history + ') ');
                
                updateHistoryScreen(history); 
               currNum = (btn === 'sqr') ? square(currNum) : (btn === '√') ? squareRoot(currNum) : fraction(currNum);
            ...
            if (isNaN(btn) && (!isNaN(prevBtn) || prevBtn === '%' || prevBtn === 'sqr' || prevBtn === '√' || prevBtn === '1/x') &&
                btn !== '=' && btn !== 'C' && btn !== 'CE' && btn !== 'CS' && btn !== '.' && btn !== '%' && btn !== 'sqr' & btn !== '√' && btn !== '1/x') {
                mathOpCount++;
            }

            // count result in row
            if (mathOpCount >= 2 && (!isNaN(prevBtn) || prevBtn === '√' || prevBtn === 'sqr' || prevBtn === '1/x' || prevBtn === '%') && btn !== 'CE' && btn !== 'CS') {
